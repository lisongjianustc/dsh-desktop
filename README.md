...
| 方式 | 探测路径 | 触发命令 |
|---|---|---|
| `auto`（默认） | 直接路径（config.dshPath → `~/Library/pnpm/dsh` → 登录 shell `command -v dsh`）→ npx 探测 | 直接命中时：`dshPath web --port 0`；都没命中时：`npx -y @deepseek-ai/dsh web --port 0` |
| `direct` | 仅上述直接路径；都失败则报"未找到 dsh" | `dshPath web --port 0` |
| `npx`   | 直接跳过；只走 npx 探测（`npx -y @deepseek-ai/dsh --version` 在 30s 内有版本输出即视为可用） | `npx -y @deepseek-ai/dsh web --port 0` |

**npx 模式适合已经把 dsh 改成 `npx @deepseek-ai/dsh` 启动的用户**：无需本机安装 dsh，npx 自动解析并下载对应版本；后端更新也由"重启 App"触发（npx 会在下次启动时拉到最新版），原"持久安装步骤"自动跳过，profile 插件更新照常运行。

后端由 `watchdog.js` 拉起，子进程以 `detached: true` 创建**新进程组**，全部信号通过 `process.kill(-pid, sig)` 作用到整个组——npx 模式下能确保 `npx → node dsh` 整个子孙链一并被回收，无孤儿子进程。
...
bash test/watchdog-group-kill.sh                                                            # watchdog 组杀测试（验证 npx 链式派生的无孤儿保证）
...
src/backend.js     后端进程管理：findDshInvocation（direct/npx 双路径）、登录 shell PATH 修复、spawn、就绪探测、重启
...
src/updater.js     更新：npm 版本比对、dsh 持久安装升级（npx 模式跳过）、profile 插件更新、后端重启
...
- **dsh 启动方式（direct / npx）**：`findDshInvocation()` 先按 `config.dshPath → ~/Library/pnpm/dsh → 登录 shell command -v dsh` 尝试直接路径；都没命中时通过 `npx -y @deepseek-ai/dsh --version` 探测 npx 可用性，按可用模式加载后端。settings 里可强制 `direct` / `npx` / `auto`。
- **更新同步**：窗口 `loadURL(http://127.0.0.1:<port>)`，升级本机 dsh 即完成桌面端同步，壳零重打包。npx 模式下跳过持久安装步骤，重启 App 时 npx 拉取最新版本；profile 插件更新照常运行。
- **无孤儿保证**：后端由 watchdog 子进程持有，且后端 PID 经 stdout 上报给主进程；Electron 退出时**双通道 SIGTERM**（直接信号后端 + 经 watchdog 转发）。watchdog 使用 `detached: true` 创建新进程组，**所有信号通过 `process.kill(-pid, sig)` 作用于整个组**——npx 模式下 `npx → node dsh` 整个子孙链一并被回收，无孤儿子进程。被强杀时 watchdog 以 2s 轮询检测父进程失联并在宽限后 SIGKILL 整个组。
