...
const DEFAULTS = {
  // 后端 dsh 启动方式：'auto' 自动（直接优先 → pnpm dlx → 旧 npx）| 'direct' 直接调用已安装的 dsh | 'pnpm' 走 pnpm dlx @deepseek-ai/dsh | 'npx' 兼容旧配置走 npx -y @deepseek-ai/dsh
  dshInvocationMode: 'auto',
  // 后端 dsh 可执行文件路径；留空 = 自动探测（~/Library/pnpm/dsh → ~/Library/pnpm/bin/dsh → 登录 shell command -v dsh）
  dshPath: '',
